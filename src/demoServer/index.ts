...
testableEnvironments.forEach(env => {
...
async function handleHome(req: BunRequest<'/:env/'>): Promise<Response> {
  ...
  if (sessionId &&
      ...
      (
          sessions.get(env)?.get(sessionId)?.expires ?? 0
      ) >
      Date.now()) {
  ...
  return new Response(htmlTemplate(`${env.charAt(0).toUpperCase() + env.slice(1)} Environment`, content), {
...
function parseCookies(cookieHeader: string): Record<string, string> {
  return cookieHeader
      .split(';')
      .map(
          cookie => cookie.trim().split('=') as [string, string]
      )
